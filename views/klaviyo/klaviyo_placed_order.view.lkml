...
view: klaviyo_placed_order {
  derived_table: {
    sql:
      SELECT order_id,order_Value, campaign,flow,order_time
      FROM
      (
        SELECT order_id, order_Value, campaign,flow, event_type,order_time, ROW_NUMBER() OVER (partition by order_id ORDER BY order_time desc) as row_num
        FROM
        (
          ...
          FROM `aerobic-datum-283623.klaviyo.event` p1
          LEFT JOIN `aerobic-datum-283623.klaviyo.event` p2
  ...
  measure: count_of_order{
    type: count_distinct
    sql: ${TABLE}.order_id ;;
    drill_fields: [date_of_order_date,order_id,order_Value]
  }
  measure: revenue {
    type: sum
    value_format: "#,##0.00"
